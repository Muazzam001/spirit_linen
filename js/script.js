...
//    checkbox handler code
    ...
    $("#privacy").one("click", checkHandle1);
     ...
     function checkHandle3() {
        $("#icon-collapse").animate({
            "display": "flex",
            "top": "60px"
    });
        $(this).one("click", checkHandle4);
    }
    function checkHandle4() {
        $("#icon-collapse").animate({
            "top": "-260px"
    });
        $(this).one("click", checkHandle3);
    }
    $(".cart-toggle").one("click", checkHandle3);

    // scroll
  var heightCounter = $('.page').height() + $('.page').offset().top;
  $("#buttonBtm").click(function() {
    $('#wrapper').animate({scrollTop: heightCounter }, 'slow');
     heightCounter += $('.page').height();
  })
   $("#buttonTop").click(function() {
    $("#wrapper").animate({scrollTop: $('#wrapper').offset().top + 0 }, 'slow');
    heightCounter = $('.page').height();
  })
